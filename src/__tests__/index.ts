...
describe("Neo4j Agent Memory Tools (Long-Term Memory)", () => {
  ...
  test("should register and execute add and search memory tools correctly", async () => {
    // 1. Re-initialize Genkit explicitly enabling the Memory Tools
    setupCtx.ai = genkit({
      plugins: [
        googleAI(),
        neo4j([
          {
            ...
            embedder: mockEmbedder,
            clientParams: setupCtx.clientParams,
            enableAgentMemoryTools: true, // ENABLEMENT
          },
        ]),
      ],
    });

    const addToolName = `neo4j/${indexId}/addMemoryEntity`;
    const searchToolName = `neo4j/${indexId}/searchMemoryEntities`;

    // Verify that the tools have been registered in the Genkit registry
    const addAction = await setupCtx.ai.registry.lookupAction(`/tool/${addToolName}`);
    const searchAction = await setupCtx.ai.registry.lookupAction(`/tool/${searchToolName}`);

    expect(addAction).toBeDefined();
    expect(searchAction).toBeDefined();

    // 2. Action: Execute the tool to add an entity
    const entityName = `TestEntity_${Date.now()}`;
    ...
    expect(addResult).toContain("saved successfully");

    // 3. Neo4j Verification: Check directly in the graph if the node exists
    // agent-memory saves LongTerm nodes using the entity type as the label
    const dbVerification = await setupCtx.session.run(
      `MATCH (n:TEST_ORG {name: $entityName}) RETURN n`,
      { entityName }
    );
    expect(dbVerification.records).toHaveLength(1);
    expect(dbVerification.records[0].get('n').properties.description).toBe("A test organization for unit testing.");

    // 4. Action: Execute the tool to search for the entity
    const searchResult: any = await searchAction!({
      query: entityName
    });
    ...
    expect(searchResult[0].name).toBe(entityName);
    expect(searchResult[0].description).toBe("A test organization for unit testing.");
  });
});